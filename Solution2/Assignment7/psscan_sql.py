...
parser.add_argument('--fasta', type=ap.FileType('r'), default=sys.stdin, required=False)
#parser.add_argument('--db', type=str, default=sys.stdin, required=False)
...
parser.add_argument('--extern', required=False)
...
if args.db != None:
    ...
    pattern = translate_pattern(prosite_pattern)
    query = """ SELECT sequence FROM sequence WHERE sequence REGEXP '%p'"""%(pattern);
